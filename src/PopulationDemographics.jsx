    ...
    <div id="card-Global" style={{background:"#fff",borderRadius:16,overflow:"hidden",boxShadow:"0 2px 16px rgba(11,31,58,0.12)",border:"1px solid "+C.border,scrollSnapAlign:"start",flexShrink:0,width:"calc(100vw - 24px)",maxWidth:390}}>
      ...
      {expanded&&(
        <div style={{padding:"12px 14px",borderTop:"1px solid "+C.border,background:"#F8FAFC"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          <GlobalCard allRegionsData={Object.fromEntries(Object.entries(REGIONS).map(([r,rd])=>[r,YEARS.map(y=>(rd.countries||[]).reduce((acc,name)=>{const cd=COUNTRIES[name];if(!cd)return acc;const row=getDataForYear(cd.data,y);if(!row)return acc;return {year:y,total:(acc.total||0)+(row.total||0),under18:(acc.under18||0)+(row.under18||0),a18to49:(acc.a18to49||0)+(row.a18to49||0),a50to64:(acc.a50to64||0)+(row.a50to64||0),a65to79:(acc.a65to79||0)+(row.a65to79||0),over80:(acc.over80||0)+(row.over80||0)};},{year:y,total:0,under18:0,a18to49:0,a50to64:0,a65to79:0,over80:0})).filter(d=>d.total>0)]))} selectedYear={selectedYear} onScrollTo={scrollToRegion} onBandClick={setExpandedBand} expandedBand={expandedBand}/>
