...
function computeProfile(dataRows, ageGroups, genderFilter, year) {
  ...
  const p    = Math.round(row.a50to64  * mod.p);
...
function CountryProfileCard({ name, data, year, ageGroups, genderFilter, onSelect, isSelected, isFav, onToggleFav, defaultOpen, cardId, onZoom }) {
  ...
  const w_now    = Math.round(rowNow.a18to49 * mod.w);
  const p_now    = Math.round(rowNow.a50to64 * mod.p);
...
export default function PopulationDemographics() {
  ...
  const tableRows = useMemo(() => {
    return activeCountries
      .map(name => {
        ...
        const w      = Math.round(rowNow.a18to49 * mod.w);
        const p      = Math.round(rowNow.a50to64 * mod.p);
  ...
  const selectedBandsValue = (row) => {
    ...
    if (ageGroups.includes("18to49"))  v += row.a18to49 * mod.w;
    if (ageGroups.includes("50to64"))  v += row.a50to64 * mod.p;
                  ...
                  <div style={{marginTop:10,display:"flex",flexDirection:"column",gap:5}}>
                    {regionDonut.map(seg=>(
                      <div key={seg.label} style={{display:"flex",alignItems:"center",gap:6}}>
                        ...
                        <span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:11,color:"rgba(255,255,255,0.85)",letterSpacing:0.5,flex:1}}>{seg.label}</span>
                        <span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:12,color:"#fff",letterSpacing:0.3}}>{fmt(seg.value)}</span>
                        <span style={{fontFamily:"system-ui",fontSize:10,color:"rgba(255,255,255,0.6)",width:36,textAlign:"right"}}>{seg.mixPct!=null?seg.mixPct.toFixed(1)+"%":""}</span>
                      </div>
                    ))}
                  </div>
                </div>

                {/* RIGHT — Age group table with CAGR */}
                {(()=>{
                  const r25 = getDataForYear(REGIONS[region]?.data||[], 2025);
                  const r30 = getDataForYear(REGIONS[region]?.data||[], 2030);
                  const bands = [
                    {label:"< 18",   k25:r25?.under18||0, k30:r30?.under18||0, col:C.green  },
                    {label:"18–49",  k25:r25?.a18to49||0, k30:r30?.a18to49||0, col:C.teal   },
                    {label:"50–64",  k25:r25?.a50to64||0, k30:r30?.a50to64||0, col:C.purple },
                    {label:"65–79",  k25:r25?.a65to79||0, k30:r30?.a65to79||0, col:C.amber  },
                    {label:"80+",    k25:r25?.over80||0,  k30:r30?.over80||0,  col:C.red    },
                  ];
                  const tot25 = r25?.total||1;
                  return (
                    <div>
                      <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:11,letterSpacing:2,color:"rgba(255,255,255,0.6)",marginBottom:8}}>AGE GROUPS · CAGR 25→30</div>
                      <div style={{display:"flex",flexDirection:"column",gap:0}}>
                        {/* Header */}
                        <div style={{display:"grid",gridTemplateColumns:"2fr 2fr 1.2fr 1.5fr",gap:4,paddingBottom:5,borderBottom:"1px solid rgba(255,255,255,0.2)",marginBottom:4}}>
                          {["BAND","2025 POP","SHARE","CAGR"].map(h=>(
                        ...
                        {/* Rows */}
                        {bands.map(b=>{
                          const gr = r30 ? cagr(b.k25, b.k30, 5) : null;
                          return (
                            <div key={b.label} style={{display:"grid",gridTemplateColumns:"2fr 2fr 1.2fr 1.5fr",gap:4,padding:"4px 0",borderBottom:"1px solid rgba(255,255,255,0.07)"}}>
                              <div style={{display:"flex",alignItems:"center",gap:5}}>
                                <div style={{width:7,height:7,borderRadius:2,background:b.col,flexShrink:0}}/>
                                <span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:12,color:"rgba(255,255,255,0.92)",letterSpacing:0.5}}>{b.label}</span>
                              </div>
                              <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:12,color:"#fff",letterSpacing:0.3,textAlign:"right"}}>{fmt(b.k25)}</div>
                              <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:11,color:"rgba(255,255,255,0.75)",textAlign:"right"}}>{(b.k25/tot25*100).toFixed(1)}%</div>
                              <div style={{textAlign:"right"}}>
                                {gr!=null && <span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:12,color:gr>0?C.green:"#FF8A8A",letterSpacing:0.3}}>{gr>0?"+":""}{gr.toFixed(2)}%</span>}
                              </div>
                            </div>
                          );
                        })}
                        {/* Total row */}
                        {r25 && r30 && (
                          <div style={{display:"grid",gridTemplateColumns:"2fr 2fr 1.2fr 1.5fr",gap:4,padding:"6px 0 2px",borderTop:"1px solid rgba(255,255,255,0.25)",marginTop:2}}>
                            ...
                            <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:11,color:"rgba(255,255,255,0.75)",textAlign:"right"}}>100%</div>
                            <div style={{textAlign:"right"}}>
                              {(()=>{const g=cagr(r25.total,r30.total,5);return <span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:12,color:g>0?C.green:"#FF8A8A"}}>{g>0?"+":""}{g.toFixed(2)}%</span>;})()}
                            </div>
                          </div>
                        )}
                      </div>
